$(function() {
    ...
    var arr2 = _.map(arr, 'skills');

    var arr3 = _.flatten(arr2);

    var arr4 = _.uniqueId(arr3);

    for(var x in arr4) {

        first = arr4[x].toLowerCase();

        arr4[x] = first;

    }

    var arr5 = _.sortBy(arr4);

    for(var i = 0; i < arr5.length; i++) {

        newDiv = document.createElement('div');

        $('.content1').append(arr5[i], newDiv);

    }

    var arr6 = _.sortBy(arr, 'friends');

    var arr7 = _.map(arr6, 'name');

    for(var i = 0; i < arr7.length; i++) {

        newDiv = document.createElement('div');

        $('.content2').append(arr7[i], newDiv);

    }

    var arr8 = _.map(arr, 'friends');

    var arr9 = _.flatten(arr8);

    var arr10 = _.map(arr9, 'name');

    var arr11 = _.uniq(arr10);

    for(var i = 0; i < arr11.length; i++) {

        newDiv = document.createElement('div');

        $('.content3').append(arr11[i], newDiv);
